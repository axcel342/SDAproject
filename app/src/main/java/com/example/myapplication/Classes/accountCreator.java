...
public class accountCreator extends dbConnection{
    ...
    public void insertAidDetails(Context ptr,String name, String details,String uname)
    {
        if(connection!=null)
        {
            Statement s1=null;
            ...
            {
                s2 = connection.createStatement();
                String query2 = "select a.idUser from [User] a join University u on a.idUser=u.idUniversity where a.userName = '"+uname+"'";
                ResultSet resultSet1 = s2.executeQuery(query2);

                while(resultSet1.next())
                {
                    idUni = resultSet1.getInt(1);
                }
            }
            catch(SQLException e)
            {
            ...
            {
                s1 = connection.createStatement();
                Log.d("sqlreturn", idUni + name + details + uname);
                String query = "insert into FinancialAid values ("+idUni+",'"+name+"','"+details+"')";
                s1.executeQuery(query);
            }
            catch(SQLException e)
            {
